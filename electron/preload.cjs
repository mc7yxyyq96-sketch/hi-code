...
contextBridge.exposeInMainWorld("hicode", {
  ...
  browserOpen: (payload) => safeInvoke("browser:open", optionalObject(payload)),
  browserClose: () => safeInvoke("browser:close"),
  browserBounds: (payload) => safeInvoke("browser:bounds", optionalObject(payload)),
  browserNavigate: (payload) => safeInvoke("browser:navigate", optionalObject(payload)),
  browserBack: () => safeInvoke("browser:back"),
  browserForward: () => safeInvoke("browser:forward"),
  browserReload: () => safeInvoke("browser:reload"),
  browserState: () => safeInvoke("browser:state"),
  onBrowserMeta: (cb) => typeof cb === "function" && ipcRenderer.on("browser:meta", (_e, d) => cb(optionalObject(d))),
  scanCodemap: (payload) => safeInvoke("codemap:scan", optionalObject(payload)),
  listMemory: (payload) => safeInvoke("memory:list", optionalObject(payload)),
  addMemory: (payload) => safeInvoke("memory:add", optionalObject(payload)),
