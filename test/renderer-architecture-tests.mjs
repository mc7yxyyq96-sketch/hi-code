...
check("real Electron E2E covers supported widths and interactions", pkg.scripts["test:electron-e2e"] === "node tests/electron-e2e/run.mjs" && electronE2e.includes("_electron as electron") && JSON.stringify(electronE2eBaseline.widths) === JSON.stringify([720, 1024, 1440]) && electronE2e.includes("baseline.widths") && electronE2e.includes("verifyNavigation") && electronE2e.includes("verifyResponsivePanels"));
