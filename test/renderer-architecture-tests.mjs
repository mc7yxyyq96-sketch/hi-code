...
check("sidebar brand keeps Hi Code visible with prerelease versions", css.includes(".brand-main") && css.includes("flex: 1 1 auto") && css.includes(".brand-label {\n  flex: 0 0 auto;") && css.includes("max-width: 86px") && bootstrap.includes("function formatSidebarVersion") && bootstrap.includes("alpha\\.(\\d+)") && bootstrap.includes("setSidebarVersion(d.version)"));
check("about dialog shows full prerelease version instead of sidebar truncation", html.includes('class="about-version" id="aboutVersion"') && !html.includes('class="brand-version" id="aboutVersion"') && css.includes(".about-version") && css.includes("max-width: none") && css.includes("overflow: visible") && bootstrap.includes("aboutVersion.title = info.version"));
...
check("integrated editor uses bundled CodeMirror with real dirty save reload and conflict actions", codeEditorSource.includes("new EditorView") && codeEditorSource.includes('key: "Mod-s"') && fileTreeSource.includes("openEditorFile") && fileTreeSource.includes("saveEditorFile") && fileTreeSource.includes('setState("conflict"') && html.includes('id="fileForceSave"'));
...
check("real Electron E2E covers supported widths editor conflicts and the review loop", pkg.scripts["test:electron-e2e"] === "node tests/electron-e2e/run.mjs" && electronE2e.includes("_electron as electron") && JSON.stringify(electronE2eBaseline.widths) === JSON.stringify([720, 1024, 1440, 1920]) && electronE2e.includes("baseline.widths") && electronE2e.includes("verifyNavigation") && electronE2e.includes("verifyTypedAppShell") && electronE2e.includes("verifyResponsivePanels") && electronE2e.includes("verifySessionKeyboardAndLongTranscript") && electronE2e.includes("verifyIntegratedEditor") && electronE2e.includes("verifyDiffCommentRevisionRequest"));
