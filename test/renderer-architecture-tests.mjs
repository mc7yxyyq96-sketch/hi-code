...
check("real Electron E2E covers supported widths editor Runtime and Git delivery loops", pkg.scripts["test:electron-e2e"] === "node tests/electron-e2e/run.mjs" && electronE2e.includes("_electron as electron") && JSON.stringify(electronE2eBaseline.widths) === JSON.stringify([720, 800, 1100, 1440, 1920]) && electronE2e.includes("baseline.widths") && electronE2e.includes("verifyNavigation") && electronE2e.includes("verifyTypedAppShell") && electronE2e.includes("verifyResponsivePanels") && electronE2e.includes("verifySessionKeyboardAndLongTranscript") && electronE2e.includes("verifyIntegratedEditor") && electronE2e.includes("verifyDiffCommentRevisionRequest") && electronE2e.includes("verifyPlanQueueAndSteer") && electronE2e.includes("verifyGitDeliveryLoop") && electronE2e.includes("verifyDesktopUxStabilization"));
