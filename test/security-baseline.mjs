...
const updateService = fs.readFileSync(path.join(root, "electron", "services", "update-service.mjs"), "utf8");
const releasePolicy = fs.readFileSync(path.join(root, "electron", "services", "release-policy.mjs"), "utf8");
const electronBuilderWrapper = fs.readFileSync(path.join(root, "scripts", "run-electron-builder.mjs"), "utf8");
const provenanceScript = fs.readFileSync(path.join(root, "scripts", "generate-provenance.mjs"), "utf8");
const releaseSourceScript = fs.readFileSync(path.join(root, "scripts", "release-source.mjs"), "utf8");
const packageSmokeScript = fs.readFileSync(path.join(root, "scripts", "package-smoke.mjs"), "utf8");
const releasePackagingWorkflow = fs.readFileSync(path.join(root, ".github", "workflows", "release-packaging.yml"), "utf8");
...
check("managed updater is packaged-only and trust-manifest gated", updateService.includes("if (!isPackaged())") && updateService.includes("validateEmbeddedReleaseManifest") && updateService.includes("unsigned_or_unapproved_build"));
check("managed updater disables silent download install and downgrade", updateService.includes("updater.autoDownload = false") && updateService.includes("updater.autoInstallOnAppQuit = false") && updateService.includes("updater.allowDowngrade = false"));
check("managed updater requires native confirmation before install", updateService.includes("dialog.showMessageBox") && updateService.includes("answer.response !== 0") && updateService.includes("userConfirmed: true"));
check("managed updater keeps Linux DEB on manual updates", updateService.includes('platform === "linux" && !appImagePath') && updateService.includes("linux_package_manual_update"));
check("release policy uses an allowlisted builder environment", releasePolicy.includes("BASE_BUILD_ENV") && releasePolicy.includes("buildReleaseChildEnv") && !electronBuilderWrapper.includes("...process.env"));
check("release policy keeps application and cloud secrets out of CI builds", releasePolicy.includes("SENSITIVE_RELEASE_KEYS") && releasePolicy.includes('policy.mode === "release" && policy.approvalRecorded') && electronBuilderWrapper.includes("releaseEnvironmentSummary"));
check("release policy fails closed for signing and publication", releasePolicy.includes("requires a signing identity") && releasePolicy.includes("requires notarization credentials") && releasePolicy.includes("Publishing requires explicit release approval"));
check("release builder rejects publish and config overrides", electronBuilderWrapper.includes("prepareBuilderArguments") && releasePolicy.includes("Builder publish mode is controlled") && releasePolicy.includes("configuration overrides are not allowed"));
check("signed package smoke validates native signatures", packageSmokeScript.includes('runChecked("codesign"') && packageSmokeScript.includes("Get-AuthenticodeSignature"));
check("release source inspection uses a minimal environment", releaseSourceScript.includes("buildReleaseGitEnv") && releaseSourceScript.includes("GIT_TERMINAL_PROMPT") && !releaseSourceScript.includes("...process.env"));
check("release provenance records source cleanliness", provenanceScript.includes("sourceTreeClean") && provenanceScript.includes("Release provenance requires a clean Git source tree"));
check("release packaging CI cannot publish", releasePackagingWorkflow.includes('HICODE_RELEASE_MODE: ci') && releasePackagingWorkflow.includes('HICODE_RELEASE_PUBLISH: "0"') && releasePackagingWorkflow.includes("permissions:\n  contents: read"));
...
check("verify script includes release pipeline tests", verifyScript.includes('"test:release-pipeline"') && verifyScript.includes("test/release-pipeline-tests.mjs"));
